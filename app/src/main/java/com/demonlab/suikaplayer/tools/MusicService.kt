...
import com.demonlab.suikaplayer.data.MusicDatabase
...
import com.demonlab.suikaplayer.audio.BalanceEffect
import com.demonlab.suikaplayer.audio.DynamicsEffect
import com.demonlab.suikaplayer.audio.LoudnessEffect
import com.demonlab.suikaplayer.audio.ReverbEffect
...
class MusicService : MediaBrowserServiceCompat() {
    ...
    companion object {
        const val ACTION_PLAY = "com.demonlab.suikaplayer.ACTION_PLAY"
        const val ACTION_PAUSE = "com.demonlab.suikaplayer.ACTION_PAUSE"
        const val ACTION_PREVIOUS = "com.demonlab.suikaplayer.ACTION_PREVIOUS"
        const val ACTION_NEXT = "com.demonlab.suikaplayer.ACTION_NEXT"
        const val ACTION_SHUFFLE = "com.demonlab.suikaplayer.ACTION_SHUFFLE"
        const val ACTION_FAVORITE = "com.demonlab.suikaplayer.ACTION_FAVORITE"
    ...
    private fun extractLyrics(song: Song) {
        ...
        serviceScope.launch(Dispatchers.IO) {
            ...
            // 0. Songs that already carry their own lyrics (e.g. the online
            // library) don't need any local file lookup.
            if (!song.lyrics.isNullOrBlank()) {
                withContext(Dispatchers.Main) {
                    playbackManager.updateLyrics(song.lyrics)
                }
                return@launch
            }
    ...
    private fun showNotification(song: Song, isPlaying: Boolean, art: android.graphics.Bitmap? = null) {
        ...
        val intent = Intent(this, SuikaPlayer::class.java).apply {
    ...
    private fun updateWidget() {
        ...
        serviceScope.launch {
            ...
            val componentName = ComponentName(applicationContext, SuikaPlayerWidgetProvider::class.java)
            ...
            if (song != null && (lastSongForRounded != song || cachedRoundedArt == null)) {
                ...
                if (art != null) {
                    ...
                    withContext(Dispatchers.IO) {
                        cachedRoundedArt = SuikaPlayerWidgetProvider.getRoundedCornerBitmap(art, 54)
                    }
                } else {
            ...
            for (appWidgetId in appWidgetIds) {
                val views = RemoteViews(packageName, R.layout.suikaplayer_widget_layout)
                ...
                if (song != null) {
                    ...
                    if (cachedRoundedArt != null) {
                        ...
                        views.setImageViewResource(R.id.widget_cover, R.drawable.ic_suikaplayer_placeholder)
                    }
                } else {
                ...
                // Open App
                val intent = Intent(applicationContext, SuikaPlayer::class.java).apply {
                    flags = Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_NEW_TASK
                }
