...
import com.demonlab.suikaplayer.tools.playlist.QueueManager
...
class PlaybackManager private constructor(private val context: Context) {
    ...
    val playbackStateSaver = PlaybackStateSaver(
        context.getSharedPreferences("suikaplayer_state", Context.MODE_PRIVATE)
    )
        ...
        managerScope.launch(Dispatchers.IO) {
            try {
                ...
                if (existing != null) {
                    ...
                    dao.insertStats(com.demonlab.suikaplayer.data.PlaybackStats(
                        id = id,
                        type = type,
    ...
    fun checkPlaylistStatus() {
        ...
        kotlinx.coroutines.MainScope().launch {
            val count = com.demonlab.suikaplayer.data.MusicDatabase.getDatabase(context).playlistDao().getPlaylistCountForSong(songId)
